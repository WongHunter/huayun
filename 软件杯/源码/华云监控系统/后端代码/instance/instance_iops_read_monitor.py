...
from chinac_way_v3.ecs.v10.instance_iops_read_monitor import InstanceIopsReadMonitor
import json
from web.save_log import save_log

def Instance_Iops_Read_Monitor(region_id, access_key_id, access_secret,start_time,end_time,id):
    profile = Profile(region_id,access_key_id,access_secret)
    client = Client(profile)
    request = InstanceIopsReadMonitor()
    request.set_id(id)
