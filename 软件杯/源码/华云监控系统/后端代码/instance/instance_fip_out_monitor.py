...
from chinac_way_v3.ecs.v10.instance_fip_out_monitor import InstanceFipOutMonitor
import json
from web.save_log import save_log

def Instance_Fip_Out_Monitor(region_id, access_key_id, access_secret,start_time,end_time,id):
    profile = Profile(region_id,access_key_id,access_secret)
    client = Client(profile)
    request = InstanceFipOutMonitor()
    request.set_id(id)
