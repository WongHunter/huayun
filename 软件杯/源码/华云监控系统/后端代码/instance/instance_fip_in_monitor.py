...
from chinac_way_v3.ecs.v10.instance_fip_in_monitor import InstanceFipInMonitor
import json
from web.save_log import save_log

def Instance_Fip_In_Monitor(region_id, access_key_id, access_secret,start_time,end_time,id):
    profile = Profile(region_id,access_key_id,access_secret)
    client = Client(profile)
    request = InstanceFipInMonitor()
    request.set_id(id)
