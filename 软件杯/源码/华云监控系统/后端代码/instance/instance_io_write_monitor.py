...
from chinac_way_v3.ecs.v10.instance_io_write_monitor import InstanceIoWriteMonitor
import json
from web.save_log import save_log

def Instance_Io_Write_Monitor(region_id, access_key_id, access_secret,start_time,end_time,id):
    profile = Profile(region_id,access_key_id,access_secret)
    client = Client(profile)
    request = InstanceIoWriteMonitor()
    request.set_id(id)
